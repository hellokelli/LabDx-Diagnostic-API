...
cases <- read_csv("scd_results.csv")
...
xgb.save(model, "sc-model.xgb")
xgb.save(model, "sc-model.json")

ggplot(wide_data_clean, aes(x = as.factor(diagnosis), y = `RDW`, fill = as.factor(diagnosis))) +
  geom_violin(alpha = 0.7) +
  geom_boxplot(width = 0.2, fill = "white") +
  labs(
    title = "RDW Distribution: Cases(SCD) vs Controls",
...
ggplot(wide_data_clean, aes(x = RDW, fill = as.factor(diagnosis))) +
  geom_histogram(alpha = 0.5, position = "identity", bins = 20) +
  labs(
    title = "RDW Distribution: Cases vs Controls",
    x = "RDW (%)",
    y = "Count",
    fill = "Diagnosis"
  ) +
...
training_features_sc <- as.data.frame(X_train_final)
write.csv(training_features_sc, "training_features_sc.csv", row.names = FALSE)
