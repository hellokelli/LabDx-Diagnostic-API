...
cases <- read_csv("sct_results.csv")
...
#Cross-Validation testing of best feature set
feature_sets <- list(
...
xgb.save(model, "sct-model.xgb")
xgb.save(model, "sct-model.json")

ggplot(wide_data_clean, aes(x = as.factor(diagnosis), y = `RDW`, fill = as.factor(diagnosis))) +
  geom_violin(alpha = 0.7) +
  geom_boxplot(width = 0.2, fill = "white") +
  labs(
    title = "RDW Distribution: Cases(SCT) vs Controls",
...
calibration_model <- glm(y_test ~ log(predictions / (1 - predictions)), family = binomial)
calibration_slope <- coef(calibration_model)[2]
calibration_intercept <- coef(calibration_model)[1]

cat("Calibration slope:", round(calibration_slope, 4), "\n")
cat("Calibration intercept:", round(calibration_intercept, 4), "\n")

training_features_sct <- as.data.frame(X_train_final)
write.csv(training_features_sct, "training_features_sct.csv", row.names = FALSE)
